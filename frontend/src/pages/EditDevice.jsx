...
export default function EditDevice() {
  ...
  useEffect(() => {
    Promise.all([fetchDevice(), fetchUsers()])
  }, [id])

  const fetchDevice = async () => {
    try {
      const response = await api.get(`/inventory/devices/${id}`)
      ...
      setFormData({
        ...
        purchaseDate: deviceData.purchaseDate ? new Date(deviceData.purchaseDate).toISOString().split('T')[0] : '',
        assignedTo: deviceData.assignedTo || ''
      })
    } catch (error) {
      toast.error('Errore nel caricamento del dispositivo')
      navigate('/inventory')
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/inventory/devices/${id}`, payload)
      ...
      if (error.response?.data?.error) {
        ...
        toast.error('Errore nell\'aggiornamento del dispositivo')
      }
    } finally {
      setLoading(false)
    }
      ...
      <ProtectedRoute roles={['admin', 'technician']}>
        <div className={ui.page}>
          <div className="text-center py-8">
            <p className="text-gray-500">Dispositivo non trovato</p>
          </div>
        </div>
      </ProtectedRoute>
    )
    ...
    <ProtectedRoute roles={['admin', 'technician']}>
      <div className={ui.page}>
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">Modifica Dispositivo</h1>
          <p className={ui.subtleText}>Modifica le informazioni del dispositivo</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow">
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-md">
              <h4 className="text-sm font-medium text-gray-700 mb-2">Informazioni dispositivo</h4>
              <div className="text-sm text-gray-600 space-y-1">
                <p><span className="font-medium">ID:</span> {device.id}</p>
                <p><span className="font-medium">Creato il:</span> {new Date(device.createdAt).toLocaleDateString('it-IT')}</p>
                <p><span className="font-medium">Ultimo aggiornamento:</span> {new Date(device.updatedAt).toLocaleDateString('it-IT')}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
              ...
              >
                ...
                {loading ? 'Aggiornamento...' : 'Aggiorna Dispositivo'}
              </button>
            </div>
          </form>
        </div>
